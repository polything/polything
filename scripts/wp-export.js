/**
 * WordPress Content Exporter (CommonJS version for testing)
 * 
...
const config = require('../config/wordpress.json');
 ...
 */
async function fetchWordPressContent(siteUrl, contentType, batchSize = DEFAULT_BATCH_SIZE) {
 ...
 */
async function processContentBatch(items, contentType, options = {}) {
  const results = {
    success: [],
    errors: [],
    total: items.length
...
async function processContentItem(item, contentType, options = {}) {
  try {
    ...
    if (!transformed.frontMatter.slug) {
      console.error(`Warning: No slug found for item ${item.id}, using fallback`);
      transformed.frontMatter.slug = `item-${item.id}`;
    }
 ...
 */
async function exportContent(siteUrl, options = {}) {
  ...
  try {
    ...
    // Get site configuration - extract domain from URL
    const siteDomain = new URL(siteUrl).hostname;
    const siteConfig = config.sites[siteDomain];
    if (!siteConfig) {
      throw new Error(`No configuration found for site: ${siteDomain}`);
    }
 ...
 */
async function generateExportReport(results, options = {}) {
